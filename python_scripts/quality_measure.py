...
def relabeling(G_new,G_old):
    ...
    for edge in G_new.edges():
        ...
        G_new_relabeled.add_edge(label1,label2, weight=G_new.edges[edge[0],edge[1]]['weight'])
...
def reweighting(G_new,G_old):
    ...
    '''
    for edge in G_new.edges():
        G_new.edges[edge[0],edge[1]]['weight']=G_old.edges[edge[0],edge[1]]['weight']
    return G_new
...
def q_measure(G,opt_triang_G, part_dict, flag_dist,min_):
    '''
