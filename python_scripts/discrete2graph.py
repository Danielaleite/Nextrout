...
def graph_filtering_from_dat_files(
    folder_name,
    t,
    graph_type,
    beta_d,
    ...
    if graph_type == "3" and weighting_method == "ER":
        ...
        for i in range(1, ncc + 1):
            for node in list(
                possible_terminals_source[i].union(possible_terminals_sink[i])
            ):
        ...
        path_fig = (
            ...
            + min_string
            + "_btns_so_"
            + btns_factor_source_string
            + "_btns_si_"
            + btns_factor_sink_string
            + "_wm"
            + str(weighting_method)
            + "_wms"
            + str(weighting_method_simplification)
        )
        plt.axis("on")
        ...
        edges_, weights_ = zip(
            *nx.get_edge_attributes(G_final_simplification, "weight").items()
        )
        ...
        nx.draw_networkx_nodes(
            terminals, pos_terminals, node_size=100, edge_color="g", node_color="g"
        )
        plt.title(
            "Weighted simplification: original ("
            + weighting_method
            + ")/ simpl ("
            + str(weighting_method_simplification)
            + ")"
        ...
        if reduction_flag == "yes":
            ...
            posGsimpl = nx.get_node_attributes(G_final_simplification, "pos")
            G_final_simplification_reduction = bifurcation_paths(
                G_final_simplification, original_label_terminals_list
            )
            deg = nx.degree_centrality(G_final_simplification_reduction)
            posGsimplred = nx.get_node_attributes(
                G_final_simplification_reduction, "pos"
            )
            ...
            nx.draw_networkx(
                G_final_simplification_reduction,
                posGsimplred,
                node_size=150,
                width=4,
                with_labels=False,
