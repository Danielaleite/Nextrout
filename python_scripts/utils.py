...
def dat2pygraph(Graph, folder_name, edge_mapping, min_, BP_weights):
    ...
    folder_name = "../muffe_sparse_optimization/simplifications/" + folder_name[2:]
...
def pygraph2dat(G, sources, sinks, component_index, folder_name, mapping, input_flag=None):
    ...
    new_dir0 = './simplifications/runs/' + folder_name[2:].split('/')[-2] + '/' + \
...
def using_graph2incidence_matrix(folder_name, index, weight_flag=None):
    ...
    folder_name_here = '../muffe_sparse_optimization/simplifications/runs/' + folder_name[2:].split('/')[-2] + '/' + \
...
def updating_beta_discrete(beta):
    ...
    f = open("../muffe_sparse_optimization/simplifications/par_files/pflux.dat", "w+")
    f.write("1  1 \n")
    f.write("time  0.0 \n")
    f.write("1 !ninputs \n")
    beta_str = str(beta)
