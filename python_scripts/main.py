...
if flag_grid != "use":
    ...
    points, vertices, coordinates, elements, element_attributes = ex_grid.example_grid(
        flag_grid, length, extra_grid
    )
...
if (len(sources) == 0) and (len(dirac_sources) == 0):
    ex_forcing.example_source(
        sources, dirac_sources, str(flag_source), str(extra_source)
    )

# append sinks or driac nodes
if (len(sinks) == 0) and (len(dirac_sinks) == 0):
    ex_forcing.example_sink(sinks, dirac_sinks, str(flag_sink), str(extra_sink))
...
if True:
    ...
    timecell.write2file(file_forcing, time, True, forcing_tria)
    timecell.write2file(file_source, time, True, forcing_tria)
    timecell.write2file(file_sink, time, True, forcing_tria)
    ...
    timecell.write2file(file_dirac_forcing, time, True, dirac_forcing_nodes)
    timecell.write2file(file_dirac_source, time, True, dirac_source_nodes)
    timecell.write2file(file_dirac_sink, time, True, dirac_sink_nodes)

    # cycle in time
    while (time <= tmax) and ((not steady_forcing) or (not steady_dirac_forcing)):
        # make source
        source_tria, steady_source = ex_forcing.make_source(
            source_tria, steady_source, sources, time, flags, bar_cell
        )
        ...
        sink_tria, steady_sink = ex_forcing.make_sink(
            sink_tria, steady_sink, sinks, time, flags, bar_cell
        )
        ...
        steady_forcing = steady_source and steady_sink
        steady_dirac_forcing = dirac_source_steady and dirac_sink_steady
        ...
        if flag_normalize == 1:
            (
                ...
                dirac_source_nodes,
                dirac_sink_nodes,
            ) = ex_forcing.normalize(
                source_tria, sink_tria, dirac_source_nodes, dirac_sink_nodes, size_cell
            )
...
while (time <= tmax) and (not steady):
    ...
    kappa_tria, steady = timecell.build(
        kappa_tria, steady, kappas, time, bar_cell, flags
    )
    # write 2 file
...
while (time <= tmax) and (not steady):
    ...
    tdens0_tria, steady = timecell.build(
        tdens0_tria, steady, tdens0_functions, time, bar_cell, flags
    )
    # write 2 file
...
if len(optdens_functions) == 0:
    optdens_functions.append(
        make_optdens.define_optimal_tdens(
            flag_grid, flag_source, flag_sink, extra_grid, extra_source, extra_sink
        )
    )
...
try:
    ...
    while (time <= tmax) and (not steady):
        ...
        optdens_tria, steady = timecell.build(
            optdens_tria, steady, optdens_functions, time, bar_cell, flags
        )
        # write 2 file
...
if len(optdens_functions) != 0:
    celldata = CellData(
        Scalars(forcing_tria[:, 0], name="forcing"),
...
vtk_object = VtkData(
    UnstructuredGrid(coord_list, triangle=topol_list), celldata, nodedata
)
...
##########################
# pflux
scalars.build_and_write(flag_pflux, extra_pflux, tzero, tmax, filename_pflux)

# pmass
scalars.build_and_write(flag_pmass, extra_pmass, tzero, tmax, filename_pmass)

# decay
scalars.build_and_write(flag_decay, extra_decay, tzero, tmax, filename_decay)
