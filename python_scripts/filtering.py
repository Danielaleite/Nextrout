...
def terminals_from_cont(Graph, source_flag, sink_flag, btns_factor_source, btns_factor_sink, terminal_criterion='branch_convex_hull+btns_centr'):
    ...
    if source_flag == 'rect_cnst':
        ...
        nodes_in_source = [node for node in Graph.nodes() if
                           fsource(Graph.nodes[node]['pos'][0], Graph.nodes[node]['pos'][1],
                                   source_flag) != 0]
    if sink_flag == 'rect_cnst':
        ...
        nodes_in_sink = [node for node in Graph.nodes() if
                         fsink(Graph.nodes[node]['pos'][0], Graph.nodes[node]['pos'][1],
                               sink_flag) != 0]
...
def BP_solver(folder_name, index):
    ...
    for folder in ["output", "output/result", "output/vtk", "output/linear_sys", "output/timefun"]:
        ...
        try:
            os.mkdir('../muffe_sparse_optimization/simplifications/' + folder_name + "/component" + str(index) + '/')
        except OSError:
            print("Creation of the directory %s failed." % (folder_name + "/component" + str(index) + '/'))

        try:
            os.mkdir('../muffe_sparse_optimization/simplifications/' + folder_name + "/component" + str(
                index) + '/' + folder)
        except OSError:
            print("Creation of the directory %s failed." % (folder_name + "/component" + str(index) + '/' + folder))
    os.system(
        'cp ../muffe_sparse_optimization/simplifications/muffa.fnames  ' + '../muffe_sparse_optimization/simplifications/' + folder_name + "/component" + str(
            index) + '/')

    # Copying the par_files
    for file in ['decay', 'pflux', 'pmass']:
        os.system(
            'cp  ../muffe_sparse_optimization/simplifications/par_files/' + file + '.dat ' + '../muffe_sparse_optimization/simplifications/' + folder_name + "/component" + str(
                index) + '/input')

    # Executing the dmk_folder.py run
    continuous_path = os.path.abspath("./")
    discrete_path = "../muffe_sparse_optimization/simplifications/"
...
def filtering_from_image(small_G_filtered, beta_d, terminals, color_dict, partition_dict, weighting_method_simplification,entry=None,folder_name=None):
    '''
    ...
    try:
        ...
        os.mkdir('../muffe_sparse_optimization/simplifications/runs/'+folder_name.split('/')[-1])
    except:
        pass


    G_final_simplification, newGraph, ncc, possible_terminals_source, possible_terminals_sink, mapping = filtering(Graph,
              folder_name+"/",
                beta_d,
              graph_type,
    ...
    nx.draw_networkx(G_final_simplification, pos, node_size=5, width=3, with_labels=False, edge_color='black', alpha=1,
                     node_color='black', ax=ax)
...
def img_pre_extr2filtering(image_path, filter_size,weighting_method_simplification, beta_d):
    '''
    ...
    for node in [terminal_list[0]]:
        ...
        G_final_simplification = filtering_from_image(Graph, beta_d, terminal_list, color_dict, partition_dict, weighting_method_simplification,
                                               entry,saving_path)
    return G_final_simplification
...
def filtering(Graph,
    ...
    newGraphList, mappingList, inv_mappingList, [components_list] = utils.pickle2pygraph(Graph, graph_type)
    ...
    for i in range(1, ncc + 1):
        ...
        if len(terminal_info) == 2:
            possible_terminals_source[i] = [inv_mapping[i][node] for node in [terminals[entry]]]
            possible_terminals_sink[i] = [inv_mapping[i][node] for node in terminals if node not in [terminals[entry]]]
        ...
        for node in G_simplification[i].nodes():
            node_in_original, _ = quality_measure.old_label(node, newGraph[i], G_simplification[i])
    ...
    for i in range(1, ncc):
        ...
        G_final_simplification = nx.disjoint_union(G_final_simplification, G_simplification[i + 1])
    ...
    G_final_simplification.remove_nodes_from(
        [x for x in G_final_simplification.nodes() if G_final_simplification.degree(x) == 0])
    ...
    if weighting_method_simplification == 'ER':
        ...
        for edge in G_final_simplification.edges():
            if deg[edge[0]] * deg[edge[1]] != 0:
                G_final_simplification.edges[(edge[0], edge[1])]['weight'] = G_final_simplification.nodes[edge[0]][
                                                                                 'weight'] / (
                                                                                     deg[edge[0]] * (N - 1)) + \
                                                                             G_final_simplification.nodes[edge[1]][
                                                                                 'weight'] / (
                                                                                     deg[edge[1]] * (N - 1))
    ...
    return G_final_simplification, newGraph, ncc, possible_terminals_source, possible_terminals_sink, mapping
...
def img2filtering(image_path, new_size, number_of_colors, t1, t2, number_of_cc, graph_type, beta_d):
    ...
    G_final_simplification = img_pre_extr2filtering(image_path, filter_size, weighting_method_simplification, beta_d)
