...
def preprocessing_cont(folder_name, min_, funct):
    ...
    if os.getcwd().split("/")[-1] == "simplifications":
        ...
        if funct == "flux":
            opt_tdens = extracting_weights(folder_name, "output/result/opt_tdens.dat")
            opt_pot = extracting_weights(
                folder_name, "output/result/opt_nrm_grad_avg.dat"
            )
    ...
    print(
        "The max "
        + funct.split("/")[-1].split(".")[0].split("_")[-1]
        + " in this case is equal to",
        max_,
        ". The chosen threshold percentage is",
        min_ * 100,
        "%",
    )
    ...
    return G_bar, G_triang, dict_seq, max_
    ### end of preprocess
...
def graph_extraction_from_dat_files(
    ...
    edges_, weights_ = zip(*nx.get_edge_attributes(Graph, "weight").items())

    ec2 = nx.draw_networkx_edges(
        Graph, pos, edge_color=weights_, width=2.0, edge_cmap=plt.cm.jet, ax=ax,
    )
    if source_flag in [
        "1_rect",
        "3_rect",
